...
func _test_capture_manifest_and_image_contract() -> void:
	...
	_check(medical_east_wall.get("position", Vector3.ZERO) == Vector3(6.5, 1.7162851, -27.0), "Medical east-wall view begins inside the moved room beside the aligned wall")
	_check(medical_east_wall.get("target", Vector3.ZERO) == Vector3(7.6, 1.4, -17.0), "Medical east-wall view looks through the south-east entrance along the aligned corridor side")
	var east_wall_threshold_x := _line_x_at_z(
		medical_east_wall.get("position", Vector3.ZERO),
		medical_east_wall.get("target", Vector3.ZERO),
		-23.0
	)
	_check(east_wall_threshold_x > 5.95 and east_wall_threshold_x < 8.05, "Medical east-wall sightline crosses the threshold through the clear entrance")
...
func _line_x_at_z(start: Vector3, target: Vector3, sample_z: float) -> float:
	var progress := (sample_z - start.z) / (target.z - start.z)
	return lerpf(start.x, target.x, progress)
