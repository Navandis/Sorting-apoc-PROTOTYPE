...
func _test_capture_manifest_and_image_contract() -> void:
	...
	_check(kitchen_turn.get("position", Vector3.ZERO) == Vector3(21.5, 1.7162851, -6.8), "Kitchen turn view starts in the east leg before the corner")
	_check(kitchen_turn.get("target", Vector3.ZERO) == Vector3(23.0, 1.4, -12.5), "Kitchen turn view looks cleanly into the north leg")
