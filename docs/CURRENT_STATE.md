...
- The deterministic seeded TAKE-only Receiving deck presenter is implemented, technically verified, live in `wing_gameplay.tscn`, and human-promoted in its tested Stage B scope. It uses private placement surfaces excluded from the sixteen ordinary functional storage surfaces. It exposes no PUT, zoning, labels, manual placement, visible grid, or F6 Receiving grid. Seeded front-to-back occlusion and existing legal support stacks are accepted presentation behavior; capacity failure remains explicit and atomic.
- Receiving FRONT is local `+Z` at the player/barrier/apron side and REAR is local `-Z` inside the lift; deterministic fill proceeds FRONT → REAR. The current approved working baseline is **3.00 m × 2.00 m usable**, **3.10 m × 2.10 m physical support**, a **0.05 m non-reservable border**, and **2.1 m Receiving-only reach**. These values remain authorable and future-tunable as catalogue, Expedition balance, stack metadata, and production-lighting evidence develops.
...
- Low functional Receiving-only pallets, trays, crates, plates or mats remain allowed future design directions. Later follow-on work may also cover reveal/shutter theatre and cue hooks, capacity sizing against better Expedition bounds, and production-lighting readability; none is implemented by this promotion.
...
Completed: Review-scene supply reuse; reusable functional modular-rack authoring; Storage Unit Orientation & Zoning Basis; Canonical Item Storage Pose & Shelf-Front Alignment; Single-Rack Fixed-Ladder Proof; Wing Modular Rack + Ladder Integration bridge; deterministic seeded TAKE-only Receiving deck presenter.

1. **Next active gate: Receiving Stage B follow-on design/refinement, to be separately scoped before implementation.**
2. Allowed follow-on topics include low functional freight pallets/trays/crates, reveal/shutter theatre and cue hooks, later capacity sizing against better Expedition bounds, and later production-lighting/readability validation. No ordering among these topics is established here.
3. Receiving Stage A remains the upstream batch/lifecycle foundation; Expedition continues to own exact batch contents while the promoted Stage B presenter exposes those exact ordinary `WorldItem`s through TAKE only.
4. Receiving Stage C physical queue pressure remains later work. Ladder promotion does not create a general climbing, jumping, movable-ladder or sliding-ladder gate.

## Explicitly deferred and known debt

Production Structural Shell and Applied Finish; full gallery furnishing; freight cage lining; Receiving Stage B follow-on refinement and Stage C; timed obligations; full save and journal implementation; Gloves and Pants source and pose work; production starting inventory; player-adjustable shelf levels; crouch; height-aware auto-placement; camera-height and wing-ceiling changes. Irregular frozen-pile presentation and a pile-local support graph are not deferred production work; that direction is rejected. Storage-unit front/back semantics, zone-editor mapping, eligible canonical item pose content, and the tested deterministic Receiving deck foundation are complete and human-promoted. Existing physical zone cells are deliberately not migrated when authored orientation changes. Full 0/90/180/270 item-display rotation remains optional UX debt; future save/load must restore placed orientation instead of recomputing it from current shelf Front.
