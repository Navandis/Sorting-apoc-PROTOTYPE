# Sorting Apocalypse Preliminary GDD v0.8
...
Living design | Greybox promoted; seeded storage integration is the next gameplay gate

Consolidates the accepted whole-wing greybox, three correction rounds, final Medical tuning and deferred abandoned-route policy. The next sequence is repository/documentation close-out, then functional shelves and pre-seeded interactive loot in the accepted wing, then physical Receiving. Greybox approval does not establish production art, furnished usability or game balance.

Companion documents: Prototype Findings v0.8; Visual Design & World-Building Direction v0.5; Receiving/Elevator MVP Architecture Design (12 September 2026).
...
### v0.8 authority and evidence boundary

This GDD remains the overall design authority. Prototype Findings v0.8 records implementation and validation evidence; Visual Design & World-Building Direction v0.5 supplies environmental rules and the accepted spatial baseline. The 12 September Receiving specification retains subsystem detail. The neutral whole-wing geometry is human-promoted; art, furnished interaction and balance remain separate gates. Explicit supersessions are recorded in Appendix C.

This update preserves v0.7 design mechanics and consolidates the supplied correction reports, final Medical evidence, developer acceptance and the new seeded-storage prerequisite. A read-only GitHub inspection is recorded separately from local Codex test claims. No new local Godot run, repository integration or gameplay migration is asserted by the document edit. [G07, P07, V04, W17, H17, A17, D17B, R17]
...
ELEVATOR -> CARRIED -> SORTING TABLE / STORAGE
                       -> ACTIVE REQUEST
                       -> KITCHEN RATION INPUT -> OVERNIGHT -> PHYSICAL RATIONS
                       -> BUNKER OPS
                       -> SALVAGER -> PHYSICAL SALVAGE -> STORAGE / WORKSHOP
                       -> INCINERATOR -> REMOVED
...
Top-level contents must be reliably visible from normal supported viewpoints without introducing jumping or ladders. Crouch remains separate deferred work, not a repair for poorly composed shelving. Final furniture depth/height and capacity percentages are tested with actual catalogue items, not treated as exact art constants. [V03 §15]
...
TEAM LOAD CAPACITY = f(team size)
OUTGOING BULK      = sum(Bulk of submitted loadout items)
RETURN CAPACITY    = Team Load Capacity - Outgoing Bulk
...
1. Select destination loot table.
2. Calculate Team Load Capacity minus outgoing Bulk.
3. Medical + Protection -> per-scavenger safety outcome.
4. Food + Hydration -> expedition duration.
5. Weapons -> returned-Bulk utilization and recruitment chance.
6. All five meters -> internal Loot Quality bias; full bonus requires every lane filled.
7. Apply only explicit lane modifiers, such as Morale to recruitment; do not alter Loot Quality indirectly.
8. Sample items to Bulk and quality targets; resolve timing, casualties, recruitment, and other events.
...
The current project is a Godot prototype with local CLI-based implementation/testing and Blender available as an optional script-assisted authoring route. Godot 4.7 is the reported development baseline for this evidence window; exact installed executable/build is verified locally before tasks. Architecture remains data-driven, deterministic and explicit about irreversible ownership rather than relying on an assumed engine path.
...
ItemDefinition (gameplay type)
  stable definition ID / display name / visual resource
  destination Utility / Bulk / primary Storage Category
  Footprint / approved Storage Pose / Stack Role / Auto Group
  Salvage Yield / separately scoped optional instance-state support

ItemInstance (physical identity)
  durable instance ID / definition reference / applicable instance state

WorldItem (representation)
  existing ItemInstance / bounded owner / context / deterministic transform

LootBatch (upstream commitment)
  unique batch ID / ordered entries / seeds / Bulk totals
  CONTENT_COMMITTED or PREPARED / profile ID and revision
  committed local transforms / remaining-entry ownership
...
RequestDefinition
  destination / trigger / deadline / utility lanes / targets / overfill / resolution

OutcomeProfile
  lane coverage curve / optional thresholds / 0% behavior / affected systems / duration / report text
