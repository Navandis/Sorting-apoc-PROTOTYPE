# Sorting Apocalypse Prototype Findings v0.10
...
Evidence window: retained prototype history through the accepted continuing gameplay scene and the promoted 20-21 September Storage Authoring Foundation: review-scene supply reuse, reusable ModularRack authoring, unit orientation/zoning, canonical item pose, and catalogue-wide human review. Historical reports retain their original evidence states; this revision records explicit supersessions and the fixed-ladder proof as the active gate.

| Companion authority | Role |
| --- | --- |
| Preliminary GDD v0.10 | Current gameplay and scope authority. |
| Visual Design & World-Building Direction v0.7 | Current world, material, service-space and storage-installation/facing authority; accepted spatial baseline, with final art and gameplay separate. |
...
Supersedes v0.9 current-status summaries while preserving its historical checkpoints. Review-scene supply reuse, reusable functional ModularRack authoring, Storage Unit Orientation & Zoning Basis, Canonical Item Storage Pose & Shelf-Front Alignment, and all 40 eligible human reviews are promoted. The fixed-ladder proof is active; no ladder or Receiving implementation is promoted.
...
Whole-wing construction continues as res://gameplay/logistics_wing/wing_gameplay.tscn, the normal development entry. It preserves accepted geometry, player/HUD, twelve functional surfaces and the saved DevelopmentSetup palette. The promoted storage authoring foundation now adds reusable functional racks, unit-owned Front semantics and canonical item alignment without changing that spatial baseline. Production furnishing, ladder implementation, physical Receiving and integrated balance remain unvalidated. Current authority: GDD v0.10 and Visual Design Direction v0.7.
...
| Crouch / ladders | Crouch remains deferred. A fixed floor-position ladder serving selected taller racks is approved for one proof. No general climbing, jumping, movable/sliding ladder, animation, visible hands, fall system or shelf-top traversal is promoted. | Run the single-rack proof and obtain a human decision. Validate yaw clamp, climb speed, selected installation geometry and human acceptability. |
...
| Footprint authoring | The 11 September 28/12 split remains historical. Current 21 September reconciliation covers all 40 eligible definitions: 21 default / 19 custom poses and 27 geometry / 13 deliberate Footprint overrides; Gloves and Pants remain blocked. | Use the canonical pose workbench for new content. Preserve explicit authored Footprint and human approval; reopen only when a source or intentional pose change invalidates dependent evidence. |
| Full game systems | Stage A data/lifecycle, neutral wing and the Storage Authoring Foundation are complete. Physical Receiving, timed obligations, economy and full-system saves remain unvalidated. | Single-rack fixed-ladder proof -> human ladder decision -> functional Receiving unless a concrete storage blocker appears, then timed obligations. |
...
These counts remain the historical 11 September checkpoint and must not be read as current. Section 20 records the later Fuel Canister drift. Section 25.5 supersedes current pose/Footprint status with the 21 September full human reconciliation: 21 default / 19 custom poses and 27 geometry / 13 deliberate overrides across the same 40 eligible definitions.
...
| 20 September: review-scene supply reuse | Human PROMOTE. The shelf review now reuses the composed DevelopmentSetup from wing_gameplay.tscn and retires the active cabinet-only fixture path. |
| 20 September: reusable ModularRack | Human PROMOTE. Shelf-empty reusable scaffold, local editable shelf wrappers, Rack02 deck calibration and F6 alignment accepted. |
| 21 September: unit orientation and zoning | Human PROMOTE. One four-state orientation per unit, stable zoning view from authored Front and semantic-to-physical mapping accepted across rack, shelf and locker. |
| 21 September: canonical item pose and catalogue review | Human PROMOTE. Canonical +Z Front, separate packing turn, combined physical parity, placed-state stability, corrected workbench and full 40-item review accepted. |
| 21 September: synchronized foundation close-out | Current promoted main reference ac335d5ccf8d3e519f1025d26ea12b971c3820d6. Fixed-ladder proof becomes the active gate. |
...
The systems roadmap remains Receiving/Elevator -> first real timed request -> expedition-produced batches -> broader Systems MVP. Bookkeeping, seeded wing integration, default entry and the Storage Authoring Foundation are complete. Before physical Receiving, the current sequence is the single-rack fixed-ladder proof and human ladder decision. The prototype source does not decide final expedition rewards. [R12 §§1-7; D12; D17B]
...
| M21 | Synchronized remote main close-out reference ac335d5ccf8d3e519f1025d26ea12b971c3820d6. |
...
Current storage-authoring evidence: docs/testing/review-scene-supply-reuse-validation.md; docs/testing/modular-rack-authoring-validation.md; docs/testing/storage-unit-orientation-zoning-validation.md; docs/testing/canonical-item-storage-pose-validation.md. Current designs: docs/superpowers/specs/2026-09-20-modular-rack-authoring-design.md; docs/superpowers/specs/2026-09-20-storage-unit-orientation-zoning-basis-design.md; docs/superpowers/specs/2026-09-21-canonical-item-storage-pose-shelf-front-alignment-design.md.
...
### 24.5 Process and next evidence - historical checkpoint superseded by Section 25.7
...
Current supersession, 21 September: review-scene supply reuse, reusable functional ModularRack authoring, Storage Unit Orientation & Zoning Basis and Canonical Item Storage Pose & Shelf-Front Alignment are complete and human-promoted. The single-rack fixed-ladder proof is now the active gate, followed by the human ladder decision and then Receiving unless a concrete storage blocker appears.
...
## 25. 20-21 September Storage Authoring Foundation closure

This section records the promoted evidence that supersedes the current-status language in Section 24 while leaving earlier checkpoints intact. The synchronized close-out reference is ac335d5ccf8d3e519f1025d26ea12b971c3820d6.

### 25.1 Review-scene supply reuse

The retained shelf ergonomics review now instantiates wing_gameplay.tscn off-tree, detaches its composed DevelopmentSetup and adds only that setup to the active review tree. Authored tables, hosts, transforms, registrar path and wing_seed_v1 identities remain the live palette authority. The cabinet-only sample fixture is no longer active; the cabinet visual remains part of the A/B/C furniture comparison. Automated checks and human review produced PROMOTE.

### 25.2 Reusable ModularRack authoring and Rack02 deck calibration

The reusable base scene is shelf-empty. An editable Levels container accepts ordinary parent-scene-local shelf wrappers, so normal add, duplicate, move and delete operations persist. One root @tool component derives frame geometry, insets, preview, collision, clearance and runtime surfaces. StoragePrototypeManager remains the runtime owner. The three-level starter is a mutable review reference, not a universal standard.

| Rack02 calibration | Promoted value |
| --- | --- |
| Deck top Y | 0.113742 m |
| Deck underside Y | 0.090407 m |
| Deck X | -0.476953864 .. 0.476953864 m |
| Deck Z | -1.875163436 .. 1.875163555 m |
| Physical thickness | 0.023335 m |
| Runtime surface origin | -0.012 m relative to wrapper |

The measured physical deck rather than the full rail/bracket AABB anchors the support plane. F6 and the local placement plane align to the gray deck and stay within its calibrated rectangle after per-edge insets and 0.10 m cell quantization.

### 25.3 Storage Unit Orientation and semantic zoning

Orientation is unit-owned. Every surface generated for one unit inherits the same non-null four-state value. The developer authors Front manually; no mesh, wall or viewpoint inference is used.

| State | Front | Right | Back | Left | Semantic size |
| --- | --- | --- | --- | --- | --- |
| 0 | +Z | +X | -Z | -X | W x D |
| 1 | +X | -Z | -X | +Z | D x W |
| 2 | -Z | -X | +Z | +X | W x D |
...
For physical size W x D and semantic coordinates u left-to-right, v back-to-front, the promoted mapping is: state 0 x=u z=v; state 1 x=v z=D-1-u; state 2 x=W-1-u z=D-1-v; state 3 x=W-1-v z=u. The inverse mapping is exact. Zoning always presents Back at top and Front at bottom from authored Front, independent of player viewpoint.

Physical grid, reservations, stacks, zone-cell storage and F6 remain unchanged. Changing orientation metadata reinterprets physical cells semantically; it does not migrate them. Cross-family human review on ModularRack, Metal Shelf and Ventilated Locker produced PROMOTE.

### 25.4 Canonical Item Storage Pose and packing separation

ItemDefinition.storage_rotation_degrees remains the full pitch/yaw/roll correction from untouched imported asset to canonical stored presentation. Canonical item Front is +Z. Runtime applies unit yaw outside authored pose/seating, then applies the existing packing boolean as one additional +90-degree turn.

| Unit state | Unit Front | Packing false | Packing true |
| --- | --- | --- | --- |
| 0 | +Z | 2x1 / 0 degrees | 1x2 / 90 degrees |
| 1 | +X | 1x2 / 90 degrees | 2x1 / 180 degrees |
| 2 | -Z | 2x1 / 180 degrees | 1x2 / 270 degrees |
| 3 | -X | 1x2 / 270 degrees | 2x1 / 0 degrees |

The table uses an asymmetric canonical 2x1 Footprint. Physical reservation parity follows the combined unit plus packing turn. Auto-placement remains canonical/front-facing first and packing alternative second. Committed items keep their transform when shelf Front changes; newly placed copies use the new Front. Future save/load must restore placed orientation rather than recompute it.

### 25.5 Pose-authoring tool correction and catalogue-wide human review

The retained fixture is res://gameplay/dev/item_storage_pose/item_storage_pose_authoring.tscn. It edits the assigned external ItemDefinition, uses the runtime StorageVisualPose path and now combines Resource.changed with a bounded editor fingerprint so in-place pitch/yaw/roll and Footprint changes refresh reliably.

The inspector exposes Authored Footprint, Suggested Canonical Footprint, suggestion warning and preview packing Footprint. Suggestion uses canonical posed bounds at 0.10 m per cell with ceil and a one-cell minimum. Apply Suggested Footprint writes x/y only and preserves Footprint z and storage_rotation_degrees. Nothing silently rewrites gameplay metadata or source GLB/FBX transforms.

Final reconciliation: 42 definitions remain present and resolvable; 40 are eligible/current; Gloves loot_000034 and Pants loot_000036 remain blocked. Eligible approvals are 21 DEFAULT_POSE_APPROVED and 19 CUSTOM_POSE_APPROVED; Footprints are 27 GEOMETRY_APPROVED and 13 OVERRIDE_APPROVED. Twenty-six ItemDefinition .tres resources contain saved human pose/Footprint edits, while all 40 eligible items were reviewed. Stack Role, Auto Group, identity and blocked-item decisions remained consistent.

The corrected workbench reduced the full eligible-catalogue pass to roughly 10 minutes versus roughly two hours for the earlier seed-in-game, review, written-instructions and Codex-edit workflow.

### 25.6 Reconciliation, regression findings and known tolerances/debt

The promoted close-out preserved source assets, scale normalization, ItemCatalog references, gameplay scenes, shelf/rack definitions, unit orientation, zoning and Rack02 calibration. The human-approved Book 2x2 override produces a measured 0.289 mm front-edge excursion in the rotated locker boundary fixture, within the documented 0.5 mm visual-bounds tolerance. Fixture position, grid and calibration checks retain their strict 0.02 mm tolerance.

- The existing Windows root-certificate-store diagnostic remains environment noise rather than a task blocker.

- The dedicated missing-clearance-context warning path remains intentional test coverage.

- The scripted editor round-trip may emit scan-aborted/RID cleanup diagnostics at process shutdown; the separate normal editor scan remains clean apart from the certificate diagnostic.

- Gloves and Pants remain blocked for source/pose work.

- Optional 0/90/180/270 display rotation remains UX debt. Existing packing remains a boolean two-state choice.

- Changing unit Front does not migrate physical zone cells. Future save/load must persist placed orientation.

### 25.7 Current gate: fixed-ladder proof

No ladder implementation is promoted. The active proof targets one fixed floor-position ladder serving selected taller rack(s), not universal ladder coverage. Its purpose is reliable vertical access to additional storage, not perfect visibility or a universal reach solution.

- Frontal approach plus forward input begins attach/climb; side-walking through the trigger must not capture.

- W/S climbs up/down independent of camera pitch; release holds current height; A/D does not move laterally while attached.

- Mouse look remains available for shelf inspection; yaw is clamped relative to the ladder. Exact clamp is Validate/provisional.

- A top stop prevents ceiling/camera clipping and does not imply rack-top dismount.

- Descending through the bottom releases; continued backward input backs away cleanly with reattach suppression.
...
- No animation, visible hands, extra interact key, sprint modifier, fall/sliding system, general mantling, jumping, general climbing or shelf-top traversal.

- A fixed ladder does not solve shelf depth, brace/rung occlusion, rear reach or every low/high shelf ergonomics problem.
