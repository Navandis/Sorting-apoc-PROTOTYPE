# Sorting Apocalypse Preliminary GDD v0.10
...
PRELIMINARY GAMEDESIGN DOCUMENT
...
Version 0.10 | 21 September 2026

Living design | Storage authoring, orientation and canonical item pose promoted; fixed-ladder proof is the active gate

Consolidates the accepted whole-wing geometry and continuing gameplay composition with promoted review-scene supply reuse, reusable functional ModularRack authoring, Storage Unit Orientation & Zoning Basis, Canonical Item Storage Pose & Shelf-Front Alignment, and the completed human review of all 40 eligible ItemDefinitions. The single-rack fixed-ladder proof is the active gate; no ladder or Receiving implementation is promoted by this edition.

Companion documents: Prototype Findings v0.10; Visual Design and World Building Direction v0.7; Receiving and Elevator MVP Architecture Design (12 September 2026).
...
### v0.10 authority and evidence boundary

This GDD remains the overall design authority. Prototype Findings v0.10 records implementation and validation evidence; Visual Design and World Building Direction v0.7 supplies environmental and storage-installation direction. The accepted in-engine wing remains the working spatial baseline. The 12 September Receiving specification retains subsystem detail, while the fixed-ladder proof and human ladder decision precede physical Receiving. Explicit supersessions are recorded in Appendices C, D and E.

This update preserves established mechanics and historical checkpoints while bringing the current record through 21 September. It promotes the storage authoring foundations, unit-owned semantic Front, canonical item-facing and packing separation, and catalogue-wide pose/Footprint review. It does not promote ladder implementation, Receiving, gallery furnishing, camera or ceiling changes, crouch, four-way display rotation, or player-adjustable shelves.
...
| Appendix E | v0.10 Storage Authoring, Orientation and Item-Pose Closure |
...
The accepted in-engine builder, saved scene and final overview are now the working dimensional authority, subordinate to later explicit decisions. Detailed Topology V3 and Basic Structural Schematic V2 remain unchanged historical intent references; do not revert approved in-engine refinements to match their pixels. Visual Direction v0.5 recorded the service spaces, Kitchen-from-B routing, Medical alignment and abandoned-stub deferral at that checkpoint; Visual Direction v0.7 is the current visual authority. [V05, A17, D16-R02]
...
Future save/load must persist each committed item's placed orientation state. Loading must restore that state and must not recompute item orientation from the storage unit's current Front. Any later four-way display rotation must also be saved explicitly.
...
| Storage Footprint | Item-authored canonical width/depth occupancy for the approved +Z-front pose. The physical reservation follows storage-unit orientation plus the separate packing turn. It remains separate from Bulk and posed height. |
...
| Canonical Storage Pose | Full pitch/yaw/roll correction stored on ItemDefinition. In the canonical stance the intended item Front faces +Z; runtime aligns that pose to unit Front before applying optional 90-degree packing. |
...
Every storage unit owns one semantic Front/Right/Back/Left state shared by all of its surfaces. State 0 is Front +Z, Back -Z, Right +X and Left -X; states 1-3 quarter-turn that basis. The physical grid remains authoritative and unrotated. New canonical placements align item Front to unit Front, while packing_rotated adds one separate +90-degree turn.
...
- Every upper member must fit within the final physical Footprint of the item directly below it after unit orientation and the optional packing turn are combined. Stack dimensions therefore do not widen upward unless automatic base promotion first installs a larger compatible new base.

- Automatic placement is stack-first within each valid zone tier. It tries canonical/front-facing placement first and the packing alternative second. It may smart-insert a compatible item at the highest valid position and may automatically promote a larger compatible incoming item to the base if the expanded reservation remains valid. Existing members keep their relative order and their committed placed orientation.
...
- Reusable ModularRack is a shelf-empty authoring scaffold. The developer adds, duplicates, moves or deletes parent-scene-local shelf wrappers under its editable Levels container; one root controller derives dimensions, insets, collision, preview and runtime surfaces. The current three-level starter is a review reference, not a universal storage standard.

- The developer owns physical installation authoring: model, placement, dimensions, level count/distribution, unit Front and ladder availability/placement. The player owns zones, labels and day-to-day organization. No player-facing free-build follows from the authoring workflow.
...
- The zoning editor always presents a surface from its authored unit Front: Back is at the top, Front at the bottom, and the semantic aspect rotates with the unit state. Player viewpoint does not alter the view.

- Semantic paint maps to the existing physical cell array. F6 and the physical grid remain diagnostics of physical truth. Changing orientation metadata reinterprets existing zone cells; it does not migrate, rotate or reorder them.
...
- Default new storage placement should present an item's human-authored canonical Front toward the storage unit's Front. A 90-degree packing choice may intentionally turn that presentation sideways.

- A committed item's orientation is stable. Later changes to shelf Front affect new placements only; they do not rotate stored items already in place.
...
Functional storage authoring may reuse one furniture grammar across installations, but each instance retains its own dimensions, edge insets, level distribution and semantic Front. Mesh appearance must not be used to infer Front, and no per-item shelf-family rotation hack may compensate for installation authoring.
...
```text
ItemDefinition (gameplay type) stable definition ID / display name / visual resource destination Utility / Bulk / primary Storage Category Footprint / approved Storage Pose / Stack Role / Auto Group Salvage Yield / separately scoped optional instance-state supportItemInstance (physical identity) durable instance ID / definition reference / applicable instance stateWorldItem (representation) existing ItemInstance / bounded owner / context / deterministic transformLootBatch (upstream commitment) unique batch ID / ordered entries / seeds / Bulk totals CONTENT_COMMITTED or PREPARED / profile ID and revision committed local transforms / remaining-entry ownership
```
...
The retained canonical workbench is res://gameplay/dev/item_storage_pose/item_storage_pose_authoring.tscn. It edits actual ItemDefinition pose metadata, exposes authored and suggested canonical Footprints plus warning and packing-preview states, and writes a suggestion only through an explicit action that preserves Footprint z and pose. All 40 eligible definitions have been human-reviewed: 21 default and 19 custom pose approvals; 27 geometry and 13 deliberate override Footprint approvals. Gloves and Pants remain blocked.
...
```text
RequestDefinition destination / trigger / deadline / utility lanes / targets / overfill / resolutionOutcomeProfile lane coverage curve / optional thresholds / 0% behavior / affected systems / duration / report text
```
...
Placed-item records must preserve the orientation chosen at commit time. Load reconstruction must restore each placed item's saved orientation and never derive it again from the current shelf Front. Optional future 0/90/180/270 display rotation expands the saved placement state rather than the ItemDefinition's canonical pose.
...
The representative storage/content gate does not establish a functional Receiving source or broader Systems MVP. The accepted wing now supplies the continuing gameplay composition, functional storage surfaces, saved editor-authored palette and default project entry. Review-scene supply reuse, reusable functional ModularRack authoring, Storage Unit Orientation & Zoning Basis, and Canonical Item Storage Pose & Shelf-Front Alignment are promoted foundations. All 40 eligible ItemDefinitions have completed human pose/Footprint review; Gloves and Pants remain blocked. The active sequence is the single-rack fixed-ladder proof, human ladder decision, then Receiving unless a concrete storage blocker appears. Full requests, saves/journals and economy remain later work.
...
Promoted storage/content + promoted whole-wing spatial baseline + continuing gameplay/default entry + review-scene supply reuse + reusable functional ModularRack authoring + Storage Unit Orientation & Zoning Basis + Canonical Item Storage Pose & Shelf-Front Alignment -> single-rack fixed-ladder proof -> human ladder decision -> functional Receiving unless a concrete storage blocker appears -> first real timed request/system slice -> expedition-produced batches -> broader Systems MVP.
...
Seeded storage integration, the continuing gameplay composition, default launch, review-scene supply reuse, reusable functional ModularRack authoring, unit orientation/zoning and canonical item pose are complete and promoted. The next bounded gate is the single-rack fixed-ladder proof, followed by the human ladder decision and then Receiving unless a concrete storage blocker appears. Preserve the old main test scene as an explicit legacy mechanics fixture and keep the neutral and shelf-ergonomics review scenes available.
...
| Fixed-ladder proof tuning | Validate exact yaw clamp, climb speed, selected installation geometry and human acceptability. The proof serves selected taller racks only and does not establish general traversal or universal reach. |

### 21.3 Current next artifacts and bounded follow-ups

Current documents are GDD v0.10, Prototype Findings v0.10 and Visual Direction v0.7. Review-scene supply reuse, reusable functional ModularRack authoring, Storage Unit Orientation & Zoning Basis, Canonical Item Storage Pose & Shelf-Front Alignment, and the 40-item human catalogue pass are complete and promoted. The current gate order is single-rack fixed-ladder proof -> human ladder decision -> Receiving unless a concrete storage blocker appears.

- Review-scene supply reuse is complete and promoted; preserve the composed DevelopmentSetup authority and retained review-scene behavior.

- Reusable functional ModularRack authoring, unit orientation/zoning and canonical item pose are complete and promoted. Preserve their authored contracts; do not reopen them as ladder implementation work.
...
| Storage unit Front | Developer-authored semantic direction shared by every surface in one storage unit. It controls zoning presentation and new canonical item alignment without rotating the physical grid. |
| Canonical item storage pose | ItemDefinition-authored full pitch/yaw/roll correction whose intended item Front faces +Z before runtime unit alignment and optional packing rotation. |
| Packing rotation | Per-placement optional +90-degree turn kept separate from unit Front and the canonical item pose. |
...
| Storage orientation state 0 | Front +Z, Back -Z, Right +X, Left -X; states 1/2/3 quarter-turn this basis. |
| Zoning view | Always viewed from authored Front: Back at top, Front at bottom; physical F6/grid state remains unchanged. |
| Canonical item Front | +Z in the ItemDefinition authoring reference; default new placement aligns it to unit Front. |
...
HISTORICAL CHECKPOINT. Appendix D retains the 20 September v0.9 state and its then-pending gates. Appendix E supersedes its current-status claims after the promoted 20-21 September foundation milestones.
...
### E.1 Reusable installation authoring

Review-scene supply reuse and reusable functional ModularRack authoring are promoted. The retained shelf ergonomics review reuses the composed DevelopmentSetup from wing_gameplay.tscn and no longer activates the cabinet-only sample fixture.

ModularRack is a shelf-empty reusable scaffold with an editable Levels container. Parent-scene-local shelf wrappers support normal add, duplicate, move and delete operations. One root @tool controller owns dimensions, per-edge usable-area insets, calibrated visuals, collision, preview and runtime surfaces. Rack02 deck calibration and F6 alignment are promoted; the three-level starter remains a review reference rather than a universal standard.

The developer owns model, installation placement, dimensions, level count/distribution, unit Front and ladder availability/placement. The player owns zones, labels and daily organization. No player-facing free-build is approved.

### E.2 Unit Front and semantic zoning

Semantic orientation belongs to the storage unit. Every surface in one unit shares its state, and the developer authors Front manually. Mesh appearance, wall location and player approach do not infer it.

| State | Front | Back | Right | Left |
| --- | --- | --- | --- | --- |
| 0 | +Z | -Z | +X | -X |
| 1 | +X | -X | -Z | +Z |
| 2 | -Z | +Z | -X | +X |
...
Zoning always presents the surface from authored Front, with Back at the top and Front at the bottom. Semantic coordinates map reversibly to the unchanged physical cell array. F6 and the physical grid remain diagnostics; changing orientation metadata does not migrate existing physical zone cells.

### E.3 Canonical item pose and packing

ItemDefinition.storage_rotation_degrees remains the full pitch/yaw/roll correction from untouched imported visual to canonical stored presentation. Canonical item Front is authored against +Z. A new default placement aligns that Front to storage-unit Front; packing_rotated remains separate and adds +90 degrees.

| Unit state | Unit Front | Canonical placement | Packing placement |
| --- | --- | --- | --- |
| 0 | +Z | 0 degrees / width x depth | 90 degrees / depth x width |
| 1 | +X | 90 degrees / depth x width | 180 degrees / width x depth |
| 2 | -Z | 180 degrees / width x depth | 270 degrees / depth x width |
| 3 | -X | 270 degrees / depth x width | 0 degrees / width x depth |

Automatic placement remains canonical/front-facing first and packing alternative second. Reservation Footprint follows the combined unit-plus-packing parity. Committed items retain their placed orientation when shelf Front later changes. Future save/load must restore that placed orientation rather than recomputing it; optional 0/90/180/270 display rotation remains deferred UX debt.

### E.4 Catalogue authoring workflow and status

The retained fixture res://gameplay/dev/item_storage_pose/item_storage_pose_authoring.tscn edits actual ItemDefinition pose metadata and refreshes live after the bounded correction. It exposes Authored Footprint, Suggested Canonical Footprint, warning state and preview packing Footprint. Apply Suggested Footprint writes x/y only while preserving z and pose. Suggestions use canonical posed bounds and the 0.10 m cell convention; authored Footprint remains explicit gameplay metadata rather than a silent mesh-derived value. Source GLB/FBX transforms remain untouched.

The catalogue contains 42 definitions. Forty are eligible and fully human-reviewed; Gloves loot_000034 and Pants loot_000036 remain blocked. Current approvals are 21 default and 19 custom poses, with 27 geometry and 13 deliberate override Footprints. Twenty-six ItemDefinition resources changed during the human pass. The workbench reduced the full eligible-catalogue pose/Footprint review to roughly 10 minutes versus roughly two hours under the prior in-game review and handoff process.

### E.5 Single-rack fixed-ladder proof boundary and current gate order

The single-rack fixed-ladder proof is the active gate. No ladder implementation is promoted. The target is a fixed floor-position ladder serving selected taller racks, not universal ladder coverage or a general reach solution.

- Frontal approach plus forward input begins attach/climb; side-walking through the trigger must not capture.

- W/S climbs up/down independent of camera pitch; releasing movement holds current height; A/D does not move laterally while attached.

- Mouse look remains available for shelf inspection, with yaw clamped relative to the ladder. Exact clamp is Validate/provisional.

- A top stop prevents ceiling/camera clipping and does not imply rack-top dismount. Descending through the bottom releases; continued backward input backs away cleanly with reattach suppression.

- The same player body and camera move together. The carried item is preserved.

- Selected held-item interaction, pickup, auto/manual placement, rotation and HUD remain available while attached where reachable. Opening modal zoning leaves the player stationary.

- Use one climb speed initially. Exact speed, selected installation geometry and human acceptability are Validate/provisional.

- No animation, visible hands, extra interact key, sprint modifier, fall/sliding system, general mantling, jumping, general climbing or shelf-top traversal is included.

- The purpose is reliable vertical access to additional storage. The ladder does not solve shelf depth, brace/rung occlusion, rear reach or every low/high shelf ergonomics problem.

Gate order: 1) single-rack fixed-ladder proof; 2) human ladder decision; 3) Receiving unless a concrete storage blocker appears.
