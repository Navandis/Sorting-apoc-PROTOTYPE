...
**M01 commit:** `4cf5ca6c11725dd5d56a2791f80d92365b0d71b4`  
...
`res://assets/environment/storage/SM_ventilated_locker.glb`, SHA-256
...
Only the second profile's X offset fraction changed in the shared owner:

| Field | Before | After |
|---|---:|---:|
| Level-2 profile | `(0.390, 0.94, 0.92, -0.10, -0.03)` | `(0.390, 0.94, 0.92, 0.005, -0.03)` |
| Identity rear grid-line clearance | -89.285 mm | +0.743 mm |
| 0.655 rear grid-line clearance | -46.482 mm | +12.487 mm |
| Identity Soda Can rear clearance | -66.426 mm | +23.602 mm |
| 0.655 Soda Can rear clearance | -23.622 mm | +35.346 mm |
| Identity capacity | 8×13 | 8×13 |
| 0.655 capacity | 5×8 | 5×8 |

The first measured candidate, 0.00, cleared representative item bounds but
left the identity grid rear edge 3.544 mm behind the panel. The geometry-bound
regression rejected it. The final 0.005 value clears the rear boundary and the
actual native/R90 rear-row samples at both ends without losing cells. Other
locker levels, all metal profiles, level heights, Z offsets, fixture transforms
and stored item scale are unchanged.
...
preservation. It deliberately does not claim that the complete grid outline is
certified against the whole flat shelf polygon, nor does it alter the existing
vertical seating convention. Full measurements and RED/GREEN evidence are in
