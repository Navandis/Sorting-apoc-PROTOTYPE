...
The fixed-ladder proof, temporary Gallery B wing integration, and deterministic seeded TAKE-only Receiving deck presenter are human-promoted in their deliberately narrow scopes. Receiving Stage A remains upstream batch/lifecycle authority: Expedition owns the exact batch contents while Receiving presents that batch without filler, omissions, replacements or loot bias. The presenter's current working baseline is **3.00 m × 2.00 m usable**, **3.10 m × 2.10 m support** with a **0.05 m border**, and **2.1 m Receiving-only reach**; these remain authorable future-tuning values. It uses private surfaces, fills local `+Z` FRONT toward local `-Z` REAR, and exposes no PUT, zoning, labels, manual placement, visible grids, F6 Receiving surface, or ordinary storage registration. The irregular frozen-pile presenter remains human-rejected. The next active gate is **Receiving Stage B follow-on design/refinement, to be separately scoped before implementation**. Ladder promotion still does not approve universal coverage, general climbing/jumping, movable/sliding ladders or shelf-top traversal.
