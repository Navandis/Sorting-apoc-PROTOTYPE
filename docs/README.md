...
The fixed-ladder proof and temporary Gallery B wing integration remain human-promoted in their deliberately narrow scopes. Receiving Stage A remains upstream batch/lifecycle authority. The irregular frozen-pile presenter is human-rejected after the real-item proof; the next active gate is **deterministic seeded TAKE-only deck presenter design**, and no production presenter has yet been implemented. Expedition owns the exact batch contents while Receiving presents that batch without filler, omissions, replacements or loot bias. Approximately `2.0 m` usable depth is provisional and width remains open. Seeded occlusion, existing legal stacks and low Receiving-only freight fixtures remain allowed design directions; this does not approve PUT, zoning, labels, manual placement, visible grids or rigid large-back/small-front ordering. Ladder promotion does not approve universal coverage, general climbing/jumping, movable/sliding ladders or shelf-top traversal.
